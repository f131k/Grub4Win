...
BOOL CRedirector::Printf(LPCTSTR pszFormat, ...)
{
	...
	va_end(argList);
        ...
        // Added by runsisi@163.com @2011/11/23
#if defined(UNICODE) || defined(_UNICODE)
#pragma warning(push)
#pragma warning(disable:4996)
        typedef char nchar;
        size_t len = wcstombs(0, strInput, 0);
        nchar* szInput = new nchar[len + 1];
        wcstombs(szInput, strInput, len + 1);
#pragma warning(pop)

        BOOL bRet = ::WriteFile(m_hStdinWrite, (LPCSTR)szInput,
							  ...
							  HANDLE hStdErr)
{
	...
	TCHAR* tszCmdLine = _tcsdup(pszCmdLine);
	// Launch the child process.
	if (!::CreateProcess(
		NULL,
		// Modified by runsisi@163.com @2011/11/23
		/*(LPTSTR)pszCmdLine,*/
		tszCmdLine,
		NULL, NULL,
		TRUE,
